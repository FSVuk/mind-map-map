...
const BORDERS_BW = path.join(__dirname, "..", "public", "images", "map_final_bw.png");
...
async function main() {
  ...
  const BRIGHT_OCEAN = [30, 110, 220];      // bright blue ocean
  const BRIGHT_LAKE = [30, 110, 220];       // same bright blue for lakes
  ...
  for (let i = 0; i < width * height; i++) {
    ...
    if (rId === -1) {
      ...
      if (lakeIds.has(rId)) {
        // Bright blue lakes
        darkOutput[i * 3] = BRIGHT_LAKE[0];
        darkOutput[i * 3 + 1] = BRIGHT_LAKE[1];
        darkOutput[i * 3 + 2] = BRIGHT_LAKE[2];
      } else if (region && region.isOcean) {
        // Bright blue ocean
        darkOutput[i * 3] = BRIGHT_OCEAN[0];
        darkOutput[i * 3 + 1] = BRIGHT_OCEAN[1];
        darkOutput[i * 3 + 2] = BRIGHT_OCEAN[2];
      } else {
        // Land regions — keep the 70s washed colors, slight darken
